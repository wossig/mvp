	...
	public partial class ContributionDetail : ContentPage
	{
		...
		protected async override void OnAppearing()
		{
			...
			try
			{
				...
				BindingSelectors();
				var additionalTechnologies = ViewModel.MyContribution?.AdditioanlTechnologies?.Select(m => m.Name).ToArray();
				RenderAdditionalTechnologiesComponents(additionalTechnologies);
			}
			catch (Exception ex)
			{
			...
			try
			{
				...
				if (ViewModel.MyContribution == null)
				{
					var model = new ContributionModel()
					{
						...
						AdditioanlTechnologies = GetSelectedAdditionalTechnologies(),
				...
				else
				{
					...
					ViewModel.MyContribution.AdditioanlTechnologies = GetSelectedAdditionalTechnologies();
		...
		/// <summary>
		/// add single control component
		/// </summary>
		/// <param name="rowIndex">the row index of layout</param>
		/// <param name="additionalCAIndex">start from 1</param>
		/// <param name="selectCA">selected contribtion area</param>
		private void AddControlToGrid(int rowIndex, int additionalCAIndex, string selectedCA = "")
		{
			var selector = new Picker() { HeightRequest = 40, TextColor = Color.Black, ClassId = "aca" + additionalCAIndex.ToString() };
			var button = new Button() { Text = "+", WidthRequest = 40, HeightRequest = 40, TextColor = Color.Black, BorderColor = Color.LightGray, BorderWidth = 1, ClassId = "aca" + additionalCAIndex.ToString() };
			...
			button.Clicked += ButtonAdditionalTechnologyClicked;
			selector.Items.Add("--Select--");
			foreach (var item in ViewModel.ContributionAreas)
			{
			...
			{
				var model = ViewModel.ContributionAreas.FirstOrDefault(m => m.Name == selectedCA);
				selector.SelectedIndex = ViewModel.ContributionAreas.IndexOf(model);
			}
			additonalACGrid.Children.Add(selector, 0, rowIndex);
			additonalACGrid.Children.Add(button, 1, rowIndex);
		}

		private void ResetAdditionalTechnologiesButtonStatus()
		{
			var controlButtonList = additonalACGrid.Children.Where(view => view is Button).Select(view => view as Button).ToList();
			var controlButtonCount = controlButtonList.Count;

			foreach (Button ctrl in controlButtonList)
			{
				if (Int32.TryParse(ctrl.ClassId.Substring(3), out int controlIndex))
				{
					//first one
					if (controlIndex == 1)
					{
						//hide control when up to limit
						if (controlIndex == _maxControlButtonCount || _maxControlButtonCount == controlButtonCount)
						{
					...
					//preivous
					if (controlIndex < _maxControlButtonCount)
					{
						ctrl.IsVisible = false;
						ctrl.Text = "+";
					}
					...
					//last one
					if (controlIndex == _maxControlButtonCount || controlIndex == controlButtonCount)
					{
						ctrl.IsVisible = true;
						ctrl.Text = "-";
					}
		...
		private void RenderAdditionalTechnologiesComponents(string[] additionalTechnologies)
		{
			if (additionalTechnologies == null || additionalTechnologies.Length == 0)
			{
				additionalTechnologies = new string[] { string.Empty };
			}

			for (var i = 0; i < additionalTechnologies.Length; i++)
			{
				AddControlToGrid(i + 1, i + 1, additionalTechnologies[i]);
			}

			ResetAdditionalTechnologiesButtonStatus();
		}

		private IList<ContributionTechnologyModel> GetSelectedAdditionalTechnologies()
		{
			var additionalCAPickers = additonalACGrid.Children.Where(view => view is Picker).Select(view => view as Picker);
			var additionalCAs = new List<ContributionTechnologyModel>();
			foreach (var picker in additionalCAPickers)
			{
				if (picker.SelectedIndex == -1) { continue; } //no item picked
				var ca = ViewModel.ContributionAreas.FirstOrDefault(m => m.Name == picker.SelectedItem.ToString());
				if (ca != null) { additionalCAs.Add(ca); }
			}

			return additionalCAs;
		}

		protected void ButtonAdditionalTechnologyClicked(object sender, EventArgs e)
		{
			var controlButton = sender as Button;
			var controlButtonList = additonalACGrid.Children.Where(view => view is Button).Select(view => view as Button).ToList();
			var controlButtonCount = controlButtonList.Count;
			if ((Int32.TryParse(controlButton.ClassId.Substring(3), out int currentControlIndex)))
			{
				//create new pair control of contribution area
				if (currentControlIndex == 1)
				{
					AddControlToGrid(rowIndex: controlButtonCount + 1, additionalCAIndex: controlButtonCount + 1);
				}
				//remove action
				else
				{
					additonalACGrid.Children.Where(view => view.ClassId == controlButton.ClassId).ToList().ForEach(view => additonalACGrid.Children.Remove(view));
				}
			}
